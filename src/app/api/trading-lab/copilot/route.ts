...
export async function POST(req: NextRequest) {
  try {
    ...
    const selection = personalKey ? { provider: "openai" as const, model: typeof body.model === "string" && body.model.startsWith("gpt-") ? body.model : getDefaultModel("openai", "analysis") } : resolveAISelection({ provider: body.provider, model: body.model, workload: "analysis" });
    const response = await callAIProvider({ ...selection, apiKey: personalKey ?? undefined, maxTokens: 900,
