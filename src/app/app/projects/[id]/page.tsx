...
export default function ProjectDetailPage() {
  ...
  async function addTask() {
    ...
    setAdding(true);
    await fetch("/api/projects/tasks", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ project_id: id, title: newTask.trim() }) });
    setNewTask("");
    setAdding(false);
    loadTasks();
  }

  async function setStatus(taskId: string, current: string) {
    const next = current === "todo" ? "in_progress" : current === "in_progress" ? "done" : "todo";
    await fetch("/api/projects/tasks", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: taskId, status: next }) });
    loadTasks();
  }

  async function markDone(taskId: string) {
    await fetch("/api/projects/tasks", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: taskId, status: "done" }) });
    loadTasks();
  }
    ...
    <div className="flex-1 overflow-auto">
      <div className="p-8 max-w-[900px]">
        ...
        <div className="flex items-center gap-2 mb-6 text-[13px]">
          <button onClick={() => router.push("/app/projects")} className="text-[#CC785C] hover:text-[#b5684e] flex items-center gap-1"><ArrowLeft size={14}/> Projects</button>
